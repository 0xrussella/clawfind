...
// Fast settings - short timeout, no retries, accept failures
const DELAY_MS = 100;
...
async function run() {
  ...
  for (const a of needsEnrichment) {
    ...
    process.stdout.write(`\r${done}/${needsEnrichment.length} (✓${success} ✗${failures}) [${elapsed}s, ${rate}/s] ${a.name.slice(0,25).padEnd(25)}`);
